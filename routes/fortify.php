...
use Laravel\Fortify\Http\Controllers\EditedProfileInformationController;
...
Route::group(['middleware' => config('fortify.middleware', ['web'])], function () {
    ...
    Route::post(RoutePath::for('login', '/login'), [LoginController::class, 'store'])
        ->middleware(array_filter([
            'guest:'.config('fortify.guard'),
            $limiter ? 'throttle:'.$limiter : null,
        ]));

    Route::post(RoutePath::for('logout', '/logout'), [LoginController::class, 'destroy'])
        ->middleware([config('fortify.auth_middleware', 'auth').':'.config('fortify.guard')])
        ->name('logout');
        Route::post('/logoutEmployer', [LoginController::class, 'logoutEmployer'])
    ...
    if (Features::enabled(Features::registration())) {
        ...
        Route::post(RoutePath::for('register', 'employee/register'), [RegisteredUserController::class, 'store'])
            ->middleware(['guest:'.config('fortify.guard')]);
        Route::post(RoutePath::for('employer-register', 'employer/register'), [EmployerRegisterController::class, 'store'])
            ->middleware(['guest:'.config('fortify.guard')]);
    }
    ...
    if (Features::enabled(Features::updateProfileInformation())) {
        Route::put(RoutePath::for('user-profile-information.update', '/user/profile-information'), [ProfileController::class, 'update'])
            ->middleware([config('fortify.auth_middleware', 'auth').':'.config('fortify.guard')])
